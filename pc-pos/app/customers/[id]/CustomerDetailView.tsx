...
export default function CustomerDetailView({ customer, orders = [], receipts = [] }: { customer: any, orders: any[], receipts: any[] }) {
  ...
  orders.forEach(o => {
    ...
    timelineItems.push({
      type: 'order',
      id: o.id,
      date: o.date ? o.date.slice(0, 16) : '---',
      title: `Đơn hàng #${o.id}`,
      status: isPaid ? 'Đã trả đủ' : 'Ghi nợ/Chưa trả đủ',
      isPaid: isPaid,
  ...
  receipts.forEach(r => {
    ...
    // Quét và giải mã phiếu thu đã hủy
    if (isDeleted) {
    ...
    timelineItems.push({
      ...
      isPaid: true,
      amount: displayAmount, // Vẫn hiển thị số tiền gốc (sẽ gạch ngang dưới UI)
  ...
  const filteredTimeline = timelineItems.filter(item => {
    ...
    const endTime = endDate ? new Date(endDate).getTime() + 86399999 : Infinity; 
                ...
                {filteredTimeline.length > 0 ? (
                  filteredTimeline.map((item, index) => (
                    <tr key={index} className={`transition-colors ${item.isDeleted ? 'bg-gray-50/80 opacity-60' : 'hover:bg-blue-50/20'}`}>
                      <td className={`px-5 py-4 text-center font-mono text-gray-400 ${item.isDeleted ? 'line-through' : ''}`}>{item.date}</td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          {item.type === 'order' ? (
                            <span className="p-1 rounded-md bg-blue-50 text-blue-600 border border-blue-100"><ShoppingBag className="w-3.5 h-3.5" /></span>
                          ) : (
                      ...
                      <td className="px-5 py-4 text-center">
                        {item.type === 'order' ? (
                          <span className={`px-2.5 py-0.5 rounded-full font-bold text-[10px] border tracking-tight ${item.isPaid ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-orange-50 text-orange-600 border-orange-100'}`}>{item.status}</span>
                        ) : (
                      ...
                      <td className={`px-5 py-4 text-right font-black font-mono text-sm tracking-tight ${item.type === 'order' ? 'text-blue-600' : (item.isDeleted ? 'text-gray-400 line-through' : 'text-emerald-600')}`}>
